    ...
    async def on_request_completed(self, message: Message, success: bool, duration: float) -> None:
        ...
        async with self._lock:
            ...
            if self.recent_response_times:
                self.metrics.average_response_time = sum(self.recent_response_times) / len(self.recent_response_times)
                ...
                async with self._lock:
                    ...
                    if avg_response_time < self.target_response_time * 0.8 and error_rate < 0.01:
                        ...
                        if new_limit != self.current_limit:
                            self.logger.info(f"Increasing limit from {self.current_limit} to {new_limit}")
                        ...
                        if new_limit != self.current_limit:
                            self.logger.info(f"Decreasing limit from {self.current_limit} to {new_limit}")
                            self.current_limit = new_limit
