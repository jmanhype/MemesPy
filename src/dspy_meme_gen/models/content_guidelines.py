    ...
    def initialize_default_guidelines(self) -> None:
        ...
        with self.session_factory() as session:
            for cat_data in default_categories:
                ...
                category = session.query(GuidelineCategory).filter_by(name=cat_data["name"]).first()
                ...
                for guide_data in cat_data["guidelines"]:
                    guideline = (
                        session.query(ContentGuideline)
                        .filter_by(category_id=category.id, name=guide_data["name"])
                        .first()
                    )
