...
async def get_meme_metadata(meme_id: str, db: Session = Depends(get_db)) -> MemeMetadataResponse:
    ...
    if not meme:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Meme not found: {meme_id}"
        )
    ...
    return MemeMetadataResponse(
        ...
        total_duration_ms=(
            meme.generation_metadata.get("total_duration_ms") if meme.generation_metadata else None
        ),
        cache_hit=(
            meme.technical_metadata.get("cache_hit", False) if meme.technical_metadata else False
        ),
...
async def get_meme_analytics(
    meme_id: str, db: Session = Depends(get_db), cache=Depends(get_cache)
) -> MemeAnalyticsResponse:
...
async def get_generation_stats(
    time_period: str, db: Session = Depends(get_db), cache=Depends(get_cache)
) -> GenerationStatsResponse:
    ...
    logs = db.query(GenerationLog).filter(GenerationLog.timestamp >= start_time).all()
    ...
    for log in logs:
        ...
        if log.model_provider:
            provider_breakdown[log.model_provider] = (
                provider_breakdown.get(log.model_provider, 0) + 1
            )
        if log.error_type:
            error_breakdown[log.error_type] = error_breakdown.get(log.error_type, 0) + 1

    # Query memes for quality metrics
    memes = db.query(MemeMetadata).filter(MemeMetadata.created_at >= start_time).all()
    ...
    if memes:
        ...
        # Cost calculations
        total_cost = sum(m.cost_metadata.get("total_cost", 0) for m in memes if m.cost_metadata)
    ...
    # Cache hit rate
    cache_hits = sum(
        1 for m in memes if m.technical_metadata and m.technical_metadata.get("cache_hit", False)
    )
...
async def search_memes_by_metadata(
    search_request: MetadataSearchRequest, db: Session = Depends(get_db)
    ...
    if search_request.created_after:
        query = query.filter(
            MemeMetadata.created_at >= datetime.fromisoformat(search_request.created_after)
        )

    if search_request.created_before:
        query = query.filter(
            MemeMetadata.created_at <= datetime.fromisoformat(search_request.created_before)
        )
    ...
    return [
        MemeMetadataResponse(
            ...
            dspy_metadata=meme.dspy_metadata or {},
            total_duration_ms=(
                meme.generation_metadata.get("total_duration_ms")
                if meme.generation_metadata
                else None
            ),
...
async def export_metadata(
    export_request: MetadataExportRequest, db: Session = Depends(get_db)
) -> Dict[str, Any]:
    ...
    if export_request.fields:
        data = [{k: v for k, v in item.items() if k in export_request.fields} for item in data]
    ...
    if export_request.format == "json":
        ...
        return {"format": "csv", "count": len(data), "data": csv_content}
...
async def get_trending_metadata(
    ...
    memes = db.query(MemeMetadata).filter(MemeMetadata.created_at >= threshold).all()
    ...
    for meme in memes:
        ...
        # Calculate trending score
        trending_score = (
            (views * 0.1 + likes * 0.3 + shares * 0.6) * time_factor * (meme.virality_score or 0.5)
        )
