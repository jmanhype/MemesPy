    ...
    async def handle(self, command: GenerateMemeCommand) -> CommandResult[UUID]:
        ...
        try:
            ...
            return CommandResult.success(
                value=meme_id, events=[started_event] + pipeline_result.get("events", [])
            )
        ...
        try:
            ...
            if image_result.status == "success":
                ...
                stage2_complete = PipelineStageCompleted(
                    ...
                    output_data={
                        "image_url": image_result.image_url,
                        "dimensions": image_event.dimensions,
                    },
                    success=True,
                    duration_ms=generation_time,
            ...
            score_request = ScoreMemeRequest(
                text=text_result.text, image_url=image_result.image_url, topic=topic, format=format
            )
            ...
            if score_result.status == "success":
                ...
                completed_event = MemeCompleted(
                    ...
                    completion_time_ms=int(
                        (datetime.utcnow() - started_event.metadata.timestamp).total_seconds()
                        * 1000
                    ),
...
class AsyncMemeRefinementCommandHandler(CommandHandler[UUID]):
    ...
    async def handle(self, command: RefineMemeCommand) -> CommandResult[UUID]:
        ...
        try:
            ...
            return CommandResult.success(value=refined_meme_id, events=[refinement_requested])
...
async def initialize_command_handlers(actor_system: ActorSystem) -> None:
    ...
    if verification_ref:
        register_command_handler(
            "VerifyMeme", AsyncMemeVerificationCommandHandler(verification_ref)
        )
...
async def handle_command(command: Command) -> CommandResult:
    ...
    if not handler:
        return CommandResult.failure(
            f"No handler for command type: {command.metadata.command_type}"
        )
