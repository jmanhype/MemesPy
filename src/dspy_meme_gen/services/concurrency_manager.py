...
class ConcurrencyManager:
    ...
    async def submit_request(
        ...
        request = GenerationRequest(
            id=request_id, topic=topic, format=format, created_at=time.time()
        )
        ...
        try:
            while not self.queue.empty():
                async with self.processing_semaphore:  # Limit concurrent processing
                    ...
                    logger.info(
                        f"Processing request (concurrent: {self.max_concurrent - self.processing_semaphore._value})"
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"Request completed successfully in {request.processing_end - request.processing_start:.1f}s"
                        )
