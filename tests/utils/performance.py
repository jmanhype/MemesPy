...
def measure_performance(threshold_ms: Optional[int] = None) -> Callable:
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        metrics = PerformanceMetrics(
            operation_name=func.__name__, execution_times=[], success_count=0, error_count=0
        )
...
async def run_concurrent_operations(
    operation: Callable[..., T], num_concurrent: int, *args: Any, **kwargs: Any
) -> List[T]:
    ...
    """
    tasks = [operation(*args, **kwargs) for _ in range(num_concurrent)]
