...
class PrivacyService:
        ...
        stmt = select(UserConsent).where(
            and_(
                ...
                or_(
                    ...
                    UserConsent.revoked_at > datetime.utcnow()
                ),
                or_(
    ...
    async def grant_consent(
        ...
        for consent_type in consent_types:
            ...
            if consent:
                ...
                consent.expires_at = datetime.utcnow() + timedelta(days=duration_days) if duration_days else None
    ...
    async def create_privacy_safe_meme(
        ...
        if user_identifier:
            ...
            has_consent = await self.check_consent(
                session, user_identifier, ConsentType.ESSENTIAL
            )
            if not has_consent:
                raise ConsentError("Essential consent required for meme generation")
        
        # Sanitize all metadata
        safe_generation_metadata = sanitize_metadata(
            meme_data.get('generation_metadata', {})
        )
    ...
    async def execute_data_deletion(
        ...
        if request.deletion_type == "all":
            ...
            deleted_counts['memes'] = await self._delete_user_memes(
    ...
    async def export_user_data(
        ...
        data["data_categories"]["memes"] = [
            meme.anonymize() for meme in memes
        ]
        
        # Consents
        consents_stmt = select(UserConsent).where(
            UserConsent.user_pseudonym == user_pseudonym
        )
    ...
    async def _delete_user_consents(
        self,
        session: AsyncSession,
        user_pseudonym: str
    ) -> int:
        """Delete all consents for a user."""
        stmt = delete(UserConsent).where(
            UserConsent.user_pseudonym == user_pseudonym
        )
        result = await session.execute(stmt)
        return result.rowcount
    
    async def _delete_user_audit_logs(
        self,
        session: AsyncSession,
        user_pseudonym: str
    ) -> int:
    ...
    async def _delete_analytics_data(
        self,
        session: AsyncSession,
        user_pseudonym: str
    ):
    ...
    async def _delete_personalization_data(
        self,
        session: AsyncSession,
        user_pseudonym: str
    ):
