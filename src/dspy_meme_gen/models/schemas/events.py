...
class PipelineEvent(BaseEvent):
    ...
    stage_results: Dict[str, Any] = Field(
        default_factory=dict, description="Results from individual stages"
    )
...
class EventBuilder:
    ...
    def __init__(self, event_type: EventType, source: str):
        ...
        self.source = source
        self.data = {"event_type": event_type, "source": source}
...
def create_message_event(
    ...
    **kwargs,
    ...
    builder = EventBuilder(event_type, source)
    return builder.with_data(
        message_id=message_id, message_type=message_type, sender=sender, receiver=receiver, **kwargs
    ).build()


def create_meme_generation_event(
    event_type: EventType, meme_id: str, user_id: str, source: str, **kwargs
) -> MemeGenerationEvent:
    """Create a meme generation event."""
    builder = EventBuilder(event_type, source)
    return builder.with_data(meme_id=meme_id, user_id=user_id, **kwargs).build()


def create_system_event(
    event_type: EventType, component: str, operation: str, source: str, **kwargs
) -> SystemEvent:
    """Create a system event."""
    builder = EventBuilder(event_type, source)
    return builder.with_data(component=component, operation=operation, **kwargs).build()
