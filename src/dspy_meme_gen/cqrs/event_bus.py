...
class EventBus:
    ...
    def subscribe(
        ...
        logger.info(
            f"Subscribed {subscriber_id} to {event_type} (async={is_async}, priority={priority})"
        )
...
class ActorEventSubscriber:
    ...
    async def subscribe(
        self, event_type: str, priority: int = 0, filters: Optional[Dict[str, Any]] = None
    ) -> str:
...
async def subscribe_actor_to_events(
    actor_ref, event_types: List[str], priority: int = 0, filters: Optional[Dict[str, Any]] = None
