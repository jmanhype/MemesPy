...
class Supervisor(Actor):
    ...
    async def spawn_child(self, actor_class: Type[Actor], name: str, *args, **kwargs) -> ActorRef:
    ...
    async def handle_child_failure(self, child_name: str, exception: Exception) -> None:
        ...
        failure_record = FailureRecord(
            actor_name=child_name, timestamp=time.time(), exception=exception
        )
        ...
        self.logger.error(f"Child actor {child_name} failed: {exception}", exc_info=True)
    ...
    async def decide_directive(
        self, supervised: SupervisedActor, exception: Exception
    ) -> SupervisorDirective:
        ...
        recent_failures = [
            f
            for f in supervised.failure_history
    ...
    async def execute_directive(
        self, child_name: str, directive: SupervisorDirective, exception: Exception
    ) -> None:
    ...
    async def _start_single_child(self, child_name: str) -> None:
        ...
        actor = supervised.actor_class(child_name, *supervised.init_args, **supervised.init_kwargs)
...
class SupervisorTree:
    ...
    def remove_supervisor(self, supervisor_name: str) -> None:
        ...
        children = [child for child, parent in self.parent_map.items() if parent == supervisor_name]
    ...
    def _build_tree_structure(self) -> Dict[str, Any]:
        ...
        def build_node(supervisor_name: str) -> Dict[str, Any]:
            children = [
                child for child, parent in self.parent_map.items() if parent == supervisor_name
            ]

            return {"name": supervisor_name, "children": [build_node(child) for child in children]}
