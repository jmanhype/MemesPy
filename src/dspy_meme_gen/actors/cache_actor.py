...
class CacheActor(Actor):
        ...
        try:
            ...
            if data is not None:
                ...
                return CacheGetResponse(
                    request_id=message.id, status="success", found=True, value=value
                )
            else:
                self.cache_misses += 1
                ...
                return CacheGetResponse(
                    request_id=message.id, status="success", found=False, value=None
                )
            ...
            if self.enable_fallback:
                ...
                return CacheGetResponse(
                    request_id=message.id, status="error", error=str(e), found=False, value=None
                )
        finally:
            # Record request duration
            duration_ms = (time.time() - start_time) * 1000
            metrics_collector.record_agent_execution(
                agent_name="cache_actor", duration_ms=duration_ms, success=True
            )
    ...
    async def handle_cachesetrequest(self, message: CacheSetRequest) -> CacheSetResponse:
        ...
        try:
            ...
            return CacheSetResponse(request_id=message.id, status="success", success=True)
            ...
            return CacheSetResponse(
                request_id=message.id, status="error", error=str(e), success=False
            )
        finally:
            # Record request duration
            duration_ms = (time.time() - start_time) * 1000
    ...
    def _update_fallback_cache(self, key: str, value: Any, expiry: Optional[float]) -> None:
        ...
        if len(self.fallback_cache) > self.max_fallback_size:
            ...
            oldest_keys = list(self.fallback_cache.keys())[
                : len(self.fallback_cache) - self.max_fallback_size
            ]
    ...
    async def invalidate(self, pattern: str) -> int:
        ...
        try:
            ...
            if self.enable_fallback:
                ...
                fallback_keys = [
                    k for k in self.fallback_cache.keys() if fnmatch.fnmatch(k, pattern)
                ]
