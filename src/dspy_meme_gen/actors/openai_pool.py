...
class ConnectionPoolConfig:
    ...
    timeout: aiohttp.ClientTimeout = field(
        default_factory=lambda: aiohttp.ClientTimeout(total=60, connect=10, sock_read=30)
    )
...
class OpenAIConnectionPool:
    ...
    def _init_metrics(self):
        ...
        self.request_duration = meter.create_histogram(
            "openai.request.duration", unit="ms", description="OpenAI API request duration"
        )

        self.request_count = meter.create_counter(
            "openai.request.count", unit="requests", description="Total OpenAI API requests"
        )

        self.error_count = meter.create_counter(
            "openai.request.errors", unit="errors", description="OpenAI API request errors"
        )

        self.retry_count = meter.create_counter(
            "openai.request.retries", unit="retries", description="OpenAI API request retries"
        )
        ...
        **kwargs,
        ...
        async with self._ensure_started():
            ...
            return await self.circuit_breaker.call(
                self._execute_with_retry, endpoint, method, data, params, headers, **kwargs
            )
    ...
    async def _execute_with_retry(
        ...
        **kwargs,
        ...
        for attempt in range(self.retry_config.max_attempts):
            try:
                return await self._make_request(url, method, data, params, headers, **kwargs)
        ...
        **kwargs,
        ...
        try:
            async with self._session.request(
                method, url, json=data, params=params, headers=request_headers, **kwargs
            ) as response:
                # Record metrics
                duration_ms = (time.time() - start_time) * 1000
                self.request_duration.record(
                    duration_ms, {"endpoint": url.split("/")[-1], "status": response.status}
                )
                self.request_count.add(
                    1, {"endpoint": url.split("/")[-1], "status": response.status}
                )
    ...
    async def create_chat_completion(
        ...
        **kwargs,
        ...
        data = {"model": model, "messages": messages, "temperature": temperature, **kwargs}
