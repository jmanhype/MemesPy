...
class TrendingMemesModel(Base):
    ...
    __table_args__ = (Index("idx_trending_window_score", "time_window", "trend_score"),)
...
class MemeListProjection(Projection):
    ...
    async def _handle_approved(self, session: AsyncSession, event: MemeApproved) -> None:
        ...
        await session.execute(
            ...
            {"meme_id": str(event.meme_id), "approved_at": event.approval_timestamp},
        )
    ...
    async def _handle_rejected(self, session: AsyncSession, event: MemeRejected) -> None:
        ...
        await session.execute(
            ...
            {"meme_id": str(event.meme_id), "rejected_at": event.rejection_timestamp},
        )
...
class TrendingProjection(Projection):
    ...
    def __init__(self, connection_string: str):
        ...
        self.subscribe(
            EventType.MEME_VIEWED.value, EventType.MEME_SHARED.value, EventType.MEME_APPROVED.value
        )
    ...
    async def _recalculate_trending(self) -> None:
        ...
        async with self.engine.begin() as conn:
            ...
            for window in ["hour", "day", "week"]:
                interval = {"hour": "1 hour", "day": "1 day", "week": "7 days"}[window]
