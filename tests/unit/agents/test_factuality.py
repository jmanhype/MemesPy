...
def mock_correction_generator() -> Mock:
    ...
    mock = Mock()
    mock.return_value = Mock(
        correction="JavaScript was created in about two weeks"
    )
    return mock
...
def factuality_agent(
    ...
    mock_correction_generator: Mock
) -> FactualityAgent:
...
@pytest.mark.parametrize("strict_mode,expected_threshold", [
    (True, 90),
    (False, 75)
])
def test_strict_mode_verification(
    ...
    concept = {
        "topic": "Programming",
        "caption": "Language Facts",
        "format": "list"
    }
    
    result = factuality_agent.forward(
        concept=concept,
        topic="programming",
...
@pytest.mark.parametrize("concept,expected_claims", [
    (
        {"topic": "Python", "caption": "Python is fast"},
        ["Python is fast"]
    ),
    (
