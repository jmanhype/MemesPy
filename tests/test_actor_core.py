...
from src.dspy_meme_gen.actors.core import Actor, Message, Request, Response, Event, ActorRef, ActorSystem
...
class TestActorBasics:
    ...
    async def test_concurrent_message_processing(self):
        ...
        requests = [
            TestRequest(content=f"message_{i}")
            for i in range(10)
        ]
        
        # Process concurrently
        responses = await asyncio.gather(*[
            actor.send(req) for req in requests
        ])
    ...
    async def test_mailbox_overflow(self):
        ...
        try:
            await asyncio.wait_for(
                mailbox.put(TestRequest(content="3")),
                timeout=0.1
            )
