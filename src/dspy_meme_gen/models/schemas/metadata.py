...
class MemeMetadataResponse(BaseModel):
    ...
    generation_metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Generation process metadata"
    )
    image_metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Image technical metadata"
    )
...
class GenerationStatsResponse(BaseModel):
    ...
    model_breakdown: Dict[str, int] = Field(
        default_factory=dict, description="Generations by model"
    )
    provider_breakdown: Dict[str, int] = Field(
        default_factory=dict, description="Generations by provider"
    )
...
class GenerationMetadata(BaseModel):
    ...
    generation_time_ms: Optional[int] = Field(
        None, description="Generation time in milliseconds", ge=0
    )
...
class ImageMetadata(BaseModel):
    ...
    compression_ratio: Optional[float] = Field(
        None, description="Compression ratio", ge=0.0, le=1.0
    )
    generation_provider: Optional[str] = Field(None, description="Image generation provider")
    storage_location: StorageLocation = Field(
        default=StorageLocation.LOCAL, description="Where image is stored"
    )
...
class DSPyMetadata(BaseModel):
    ...
    prompts_used: Dict[str, str] = Field(
        default_factory=dict, description="Prompts used in generation"
    )
    ...
    optimization_metrics: Dict[str, float] = Field(
        default_factory=dict, description="Optimization metrics"
    )
...
class ContentAnalysis(BaseModel):
    ...
    entities_mentioned: List[Dict[str, str]] = Field(
    ...
    readability_score: Optional[float] = Field(
        None, description="Readability score", ge=0.0, le=1.0
    )
    meme_elements: List[Dict[str, str]] = Field(
        default_factory=list, description="Meme visual elements"
    )
...
class InteractionMetadata(BaseModel):
    ...
    avg_view_duration: Optional[float] = Field(
        None, description="Average view duration in seconds", ge=0.0
    )
    click_through_rate: Optional[float] = Field(
        None, description="Click-through rate", ge=0.0, le=1.0
    )
...
class ModerationMetadata(BaseModel):
    ...
    moderation_status: ModerationStatus = Field(
        default=ModerationStatus.PENDING, description="Moderation status"
    )
    flagged_categories: List[str] = Field(
        default_factory=list, description="Flagged content categories"
    )
    moderation_scores: Dict[str, float] = Field(
        default_factory=dict, description="Moderation scores"
    )
    ...
    moderation_actions: List[Dict[str, str]] = Field(
        default_factory=list, description="Actions taken"
    )
    auto_moderation_confidence: Optional[float] = Field(
        None, description="Auto-moderation confidence", ge=0.0, le=1.0
    )
