...
class ConsentRequest(BaseModel):
    ...
    duration_days: Optional[int] = Field(
        365, description="Consent duration in days", ge=1, le=730  # Max 2 years
    )
...
class DataDeletionRequest(BaseModel):
    ...
    deletion_type: str = Field(
        "all", description="Type of deletion", pattern="^(all|specific_data|specific_period)$"
    )
    data_categories: Optional[List[str]] = Field(
        None, description="Specific data categories to delete"
    )
...
async def get_privacy_settings(
    db: AsyncSession = Depends(get_db), user_id: Optional[str] = Depends(get_current_user_id)
):
...
async def revoke_consent(
    ...
    try:
        ...
        result = await privacy_service.revoke_consent(db, user_id, request.consent_types)
...
async def confirm_data_deletion(
    request_id: str, verification_token: str, db: AsyncSession = Depends(get_db)
):
    """Confirm and execute data deletion request."""
    try:
        result = await privacy_service.execute_data_deletion(db, request_id, verification_token)

        return {"status": "success", "message": "Data deletion completed", "details": result}
...
async def get_privacy_policy():
    ...
    return {
        ...
        "contact": {"email": "privacy@memeservice.com", "dpo_email": "dpo@memeservice.com"},
        "data_controller": {
...
async def run_privacy_cleanup(db: AsyncSession = Depends(get_db), api_key: str = Header(None)):
    ...
    try:
        ...
        return {"status": "success", "deleted": result, "timestamp": datetime.utcnow().isoformat()}
