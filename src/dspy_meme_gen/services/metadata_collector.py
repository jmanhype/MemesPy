...
class MetadataCollector:
    ...
    def start_generation(self, topic: str, format: str, **kwargs) -> str:
        ...
        self.generation_id = hashlib.md5(f"{topic}:{format}:{time.time()}".encode()).hexdigest()
        ...
        self.metadata = {
            ...
            "request_metadata": {"topic": topic, "format": format, "additional_params": kwargs},
            "generation_steps": [],
        }
        ...
        try:
            ...
            with Image.open(image_path) as img:
                ...
                if exif_data:
                    ...
                    image_metadata["exif_tags"] = {TAGS.get(k, k): v for k, v in exif_data.items()}
                else:
    ...
    def add_cost_tracking(
        self, text_cost: float = 0.0, image_cost: float = 0.0, storage_cost: float = 0.0
    ) -> None:
        ...
        self.metadata["cost_metadata"]["total_cost"] = (
            sum(self.metadata["cost_metadata"].values())
            - self.metadata["cost_metadata"]["total_cost"]
        )

    def add_moderation_result(
        self, status: str, scores: Dict[str, float], flagged_categories: List[str] = None
    ) -> None:
    ...
    def finalize(
        ...
        retry_count = sum(
            1
            for step in self.metadata.get("generation_steps", [])
    ...
    def embed_in_image_exif(self, image_path: str, output_path: Optional[str] = None) -> bool:
        ...
        try:
            ...
            exif_dict = {
                ...
                "Exif": {
                    piexif.ExifIFD.DateTimeOriginal: datetime.utcnow().strftime(
                        "%Y:%m:%d %H:%M:%S"
                    ),
                    piexif.ExifIFD.UserComment: json.dumps(
                        {
                            ...
                            "provider": self.metadata.get("image_generation", {}).get(
                                "final_provider"
                            ),
                            "generation_time_ms": self.metadata.get("generation_metadata", {}).get(
                                "total_duration_ms"
                            ),
                        }
                    ).encode("utf-8"),
