...
class ActorRef:
    ...
    async def ask(self, message: Message, timeout: int = 5000) -> Any:
        ...
        try:
            ...
            result = await asyncio.wait_for(future, timeout=timeout / 1000)  # Convert to seconds
            return result
    ...
    async def _handle_message(self, message: Message) -> None:
        ...
        try:
            ...
            if isinstance(message, Request) and message.reply_to:
                response = Response(request_id=message.id, status="success")
            ...
            if isinstance(message, Request) and message.reply_to:
                await message.reply_to.tell(
                    Response(request_id=message.id, status="error", error=str(e))
                )
            raise
