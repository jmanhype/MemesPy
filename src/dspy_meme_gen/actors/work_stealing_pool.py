...
class WorkStealingWorker(Actor):
    ...
    async def _process_work(self, work_item: WorkItem) -> None:
        ...
        try:
            ...
            self.logger.debug(f"Processed work item {work_item.id} in {processing_time:.3f}s")
...
class WorkStealingPool:
    ...
    async def add_worker(
        self, worker_name: str, max_queue_size: int = 1000, steal_threshold: int = 10
    ) -> ActorRef:
        ...
        worker = WorkStealingWorker(
            worker_name, self, max_queue_size=max_queue_size, steal_threshold=steal_threshold
        )
    ...
    async def submit_work(
        ...
        work_item = WorkItem(id=message.id, message=message, priority=priority, timeout=timeout)
    ...
    async def find_steal_victims(
        self, thief: WorkStealingWorker, max_victims: int
