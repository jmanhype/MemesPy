...
@router.get("/")
async def check_health(health_check: HealthCheck = Depends(get_health_check)) -> Dict[str, str]:
...
async def readiness(health_check: HealthCheck = Depends(get_health_check)) -> Dict[str, str]:
