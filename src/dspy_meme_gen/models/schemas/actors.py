...
class Request(BaseMessage):
    ...
    expects_response: bool = Field(
        default=True, description="Whether this request expects a response"
    )
...
class GenerateMemeRequest(Request):
    ...
    verification_level: str = Field(
        default="standard", description="Level of verification required"
    )
...
class GenerateMemeResponse(Response):
    ...
    refinement_iterations: int = Field(
        default=0, description="Number of refinement iterations performed"
    )
    generation_details: Dict[str, Any] = Field(
        default_factory=dict, description="Generation process details"
    )
...
class VerifyContentRequest(Request):
    ...
    verification_config: Dict[str, Any] = Field(
        default_factory=dict, description="Verification configuration"
    )
...
class VerificationCompleteEvent(Event):
    ...
    verification_results: Dict[str, Dict[str, Any]] = Field(
...
class ScoreMemeRequest(Request):
    ...
    verification_results: Dict[str, Any] = Field(
        default_factory=dict, description="Previous verification results"
    )
    scoring_criteria: List[str] = Field(
        default_factory=list, description="Specific criteria to evaluate"
    )
...
class RefineMemeResponse(Response):
    ...
    refinement_details: Dict[str, Any] = Field(
        default_factory=dict, description="Detailed refinement information"
    )
...
class RouteRequest(Request):
    ...
    routing_preferences: Dict[str, Any] = Field(
        default_factory=dict, description="Routing preferences"
    )
...
class CollectMetricsResponse(Response):
    ...
    collection_duration_ms: Optional[float] = Field(
        None, description="Time taken to collect metrics"
    )
