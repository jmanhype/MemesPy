...
from sqlalchemy import Column, String, Integer, DateTime, JSON, Index, text, select, and_, or_, func
...
class EventStore(ABC):
    ...
    async def get_events(
        self, aggregate_id: UUID, from_version: int = 0, to_version: Optional[int] = None
    ) -> List[DomainEvent]:
    ...
    async def get_all_events(
        self, from_position: int = 0, limit: Optional[int] = None
    ...
    async def get_snapshot(
        self, aggregate_id: UUID, max_version: Optional[int] = None
    ) -> Optional[Snapshot]:
...
class PostgresEventStore(EventStore):
    ...
    def __init__(self, connection_string: str):
        self.engine = create_async_engine(
            connection_string, pool_size=20, max_overflow=40, pool_pre_ping=True
        )
    ...
    async def get_all_events(
        ...
        while True:
            async with AsyncSession(self.engine) as session:
                query = (
                    select(EventEntity)
                    .where(EventEntity.global_position >= current_position)
                    .order_by(EventEntity.global_position)
                    .limit(batch_size)
                )
    ...
    async def get_events_by_type(
        ...
        """Get events by type within time range."""
        async with AsyncSession(self.engine) as session:
            query = select(EventEntity).where(EventEntity.event_type == event_type)
    ...
    async def get_snapshot(
        ...
        async with AsyncSession(self.engine) as session:
            ...
            result = await session.execute(
                query, {"aggregate_id": str(aggregate_id), "max_version": max_version}
            )
