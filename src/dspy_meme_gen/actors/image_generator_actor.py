...
class ImageGeneratorActor(Actor):
    ...
    def __init__(
        ...
        self.openai_breaker = CircuitBreaker(
            "openai_images", CircuitBreakerConfig(failure_threshold=3, timeout=60)
        )
        self.cloudinary_breaker = CircuitBreaker(
            "cloudinary_upload", CircuitBreakerConfig(failure_threshold=2, timeout=30)
        )
        ...
        self.image_renderer = ImageRenderingAgent(
            api_key=settings.openai_api_key, image_service="openai"
        )
    ...
    async def handle_generateimagerequest(self, message: GenerateImageRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            image_prompt = await self._generate_image_prompt(
                message.text, message.format, message.style
            )

            # Generate image
            self.state = ImageGenerationState.GENERATING_IMAGE
            image_data = await self._generate_image(image_prompt, message.size, message.quality)

            # Post-process if needed
            if message.add_text_overlay:
                self.state = ImageGenerationState.POST_PROCESSING
                image_data = await self._add_text_overlay(image_data, message.text, message.format)
            ...
            self.logger.info(
                f"Image generation completed in {generation_time:.2f}s " f"(CDN: {cdn_uploaded})"
            )
    ...
    async def handle_processimagerequest(self, message: ProcessImageRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            for operation in message.operations:
                if operation == "overlay":
                    image_data = await self._add_text_overlay(
                        image_data, message.text, message.format
                    )
            ...
            return {"image_data": image_b64, "operations_applied": message.operations}
    ...
    async def _generate_image_prompt(
        self, meme_text: str, format_name: str, style: Optional[str]
    ) -> str:
        ...
        try:
            ...
            base_prompt += (
                ", high quality, clear, suitable for meme text overlay, 1024x1024 resolution"
            )
    ...
    async def _generate_image(
        ...
        try:
            ...
            try:
                result = self.image_renderer.forward(image_prompt=prompt)
    ...
    async def _add_text_overlay(self, image_data: bytes, text: str, format_name: str) -> bytes:
        ...
        try:
            ...
            result = self.image_renderer.add_text_overlay(
                image_data=image_data, caption=text, position="bottom"  # Default position
            )
    ...
    def _update_average_time(self) -> None:
        ...
        if total > 0:
            self.metrics["average_generation_time"] = self.metrics["total_generation_time"] / total
    ...
    async def handle_clearcacherequest(self, message: Request) -> Dict[str, Any]:
        ...
        return {"cache_cleared": cache_size}
