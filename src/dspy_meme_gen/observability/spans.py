...
class SpanManager:
    ...
    def create_span(
        ...
        tracer = get_tracer()
        span = tracer.start_span(name=name, kind=kind, attributes=attributes, links=links)
...
@contextmanager
def api_span(operation: str, method: str, path: str, headers: Optional[Dict[str, str]] = None):
...
@contextmanager
def pipeline_span(
    stage: str, topic: str, format: Optional[str] = None, iteration: Optional[int] = None
):
    """Create span for pipeline operations."""
    attributes = {SpanAttributes.PIPELINE_STAGE: stage, SpanAttributes.MEME_TOPIC: topic}
    ...
    with trace_span(f"{SpanDefinitions.PIPELINE_STAGE}.{stage}", attributes=attributes) as span:
        yield span


@contextmanager
def agent_span(agent_name: str, operation: str, input_data: Optional[Dict[str, Any]] = None):
    """Create span for agent operations."""
    attributes = {SpanAttributes.AGENT_NAME: agent_name, SpanAttributes.AGENT_OPERATION: operation}

    if input_data:
        attributes["agent.input"] = str(input_data)[:1000]  # Limit size

    with trace_span(f"{SpanDefinitions.AGENT_EXECUTE}.{agent_name}", attributes=attributes) as span:
        yield span


@contextmanager
def db_span(operation: str, table: str, query: Optional[str] = None):
    ...
    with trace_span(
        f"{SpanDefinitions.DB_QUERY}.{operation}", kind=SpanKind.CLIENT, attributes=attributes
    ) as span:
        yield span


@contextmanager
def cache_span(operation: str, cache_type: str, key: str, hit: Optional[bool] = None):
    ...
    with trace_span(
        f"{SpanDefinitions.CACHE_GET}.{operation}", kind=SpanKind.CLIENT, attributes=attributes
    ) as span:
        yield span


@contextmanager
def external_service_span(service: str, operation: str, url: Optional[str] = None):
    ...
    with trace_span(
        f"external.{service}.{operation}", kind=SpanKind.CLIENT, attributes=attributes
    ) as span:
        yield span


def trace_api_endpoint(operation: str, tags: Optional[List[str]] = None) -> Callable[[F], F]:
...
def trace_pipeline_stage(stage: str) -> Callable[[F], F]:
...
def trace_database_operation(operation: str) -> Callable[[F], F]:
...
def trace_cache_operation(operation: str, cache_type: str = "redis") -> Callable[[F], F]:
...
def trace_external_call(service: str) -> Callable[[F], F]:
...
def trace_actor_message(message_type: str) -> Callable[[F], F]:
...
def trace_actor_lifecycle(lifecycle_event: str) -> Callable[[F], F]:
...
@contextmanager
def actor_supervision_span(supervisor_name: str, supervised_actor: str, supervision_action: str):
...
def trace_meme_generation_pipeline(stage: str) -> Callable[[F], F]:
                ...
                try:
                    ...
                    if isinstance(result, dict):
                        if stage == "scoring" and "score" in result:
                            ...
                            span.set_attribute(
                                SpanAttributes.REFINEMENT_COUNT, result["iterations"]
                            )
