...
class MemeEventStore(PostgresEventStore):
    ...
    async def get_meme_generation_stream(
        self, request_id: UUID, from_position: int = 0
    ) -> List[DomainEvent]:
    ...
    async def get_meme_pipeline_events(
        self, meme_id: UUID, stage_filter: Optional[List[str]] = None
    ) -> List[DomainEvent]:
        ...
        if stage_filter:
            pipeline_event_types = [
                t for t in pipeline_event_types if any(s in t for s in stage_filter)
            ]
        ...
        for event_type in pipeline_event_types:
            ...
            # Filter by meme_id
            filtered_events = [
                e for e in type_events if hasattr(e, "meme_id") and e.meme_id == meme_id
            ]
    ...
    async def get_meme_metrics(
        self, from_timestamp: Optional[datetime] = None, to_timestamp: Optional[datetime] = None
    ) -> Dict[str, Any]:
        """Get metrics about meme generation."""
        # Count different event types
        metrics = {
            "total_requests": len(
                await self.get_events_by_type(
                    "meme.generation.started", from_timestamp, to_timestamp
                )
            ),
            "successful_generations": len(
                await self.get_events_by_type("meme.generated", from_timestamp, to_timestamp)
            ),
            ...
            "completed_pipelines": len(
                await self.get_events_by_type("meme.completed", from_timestamp, to_timestamp)
            ),
            "approvals": len(
                await self.get_events_by_type("meme.approved", from_timestamp, to_timestamp)
            ),
            "rejections": len(
                await self.get_events_by_type("meme.rejected", from_timestamp, to_timestamp)
            ),
        }
    ...
    async def rebuild_projections(
        self, from_position: int = 0, event_types: Optional[List[str]] = None
    ) -> None:
        ...
        async for event in self.get_all_events(from_position):
            ...
            for handler in handlers:
                try:
                    ...
                    logger.error(
                        f"Projection handler error for event {event.metadata.event_id}: {e}"
                    )
