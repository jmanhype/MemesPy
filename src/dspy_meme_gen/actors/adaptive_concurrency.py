...
class AdaptiveConcurrencyController(FlowController):
    ...
    async def _adapt_gradient_descent(self) -> None:
        ...
        new_concurrency = max(
            ...
            min(
                self.metrics.max_concurrency,
                self.metrics.current_concurrency + int(adjustment)
            )
        )
    ...
    async def _adapt_additive_increase(self) -> None:
        ...
        if latest_window.error_rate > 0.01 or latest_window.average_latency > self.target_latency * 1.5:
            # Multiplicative decrease
            new_concurrency = max(
                self.metrics.min_concurrency,
                int(self.metrics.current_concurrency * 0.7)
            )
    ...
    async def _adapt_multiplicative_decrease(self) -> None:
        ...
        if latest_window.error_rate > 0.01 or latest_window.average_latency > self.target_latency * 1.2:
            # Multiplicative decrease
            new_concurrency = max(
                self.metrics.min_concurrency,
    ...
    async def _adapt_cubic(self) -> None:
        ...
        # Detect congestion
        if latest_window.error_rate > 0.01 or latest_window.average_latency > self.target_latency * 1.3:
    ...
    async def _update_concurrency(
        ...
        new_concurrency: int,
        reason: str,
        signal: Optional[AdaptationSignal] = None
    ) -> None:
    ...
    def get_concurrency_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "adaptation": {
                ...
                "direction": self.metrics.adaptation_direction.value if self.metrics.adaptation_direction else None
            },
            "pid_control": {
