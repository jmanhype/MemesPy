...
class ResourceConstrainedActor(Actor):
    ...
    async def _handle_message(self, message: Message) -> None:
        ...
        if self.current_memory_mb > self.max_memory_mb:
            ...
            raise MemoryError(f"Memory limit exceeded: {self.current_memory_mb}MB > {self.max_memory_mb}MB")
            
        if self.current_cpu_percent > self.max_cpu_percent:
            self.resource_violations += 1
            raise RuntimeError(f"CPU limit exceeded: {self.current_cpu_percent}% > {self.max_cpu_percent}%")
...
class TestChaosEngineering:
    ...
    async def test_network_partition_simulation(self):
        ...
        async def send_with_partition(actor, message_id):
            message = ChaosTestMessage(
                payload=f"partition_test_{message_id}",
                delay_ms=int(partition_duration * 1000)
            )
...
class TestFaultInjection:
    ...
    async def test_message_queue_overflow(self):
        ...
        for i in range(overflow_count):
            message = ChaosTestMessage(
                payload=f"overflow_{i}",
                delay_ms=10  # Each message takes 10ms to process
            )
        ...
        try:
            await asyncio.wait_for(
                asyncio.gather(*send_tasks[:100]),  # Only wait for first 100
                timeout=2.0
            )
