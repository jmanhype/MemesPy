...
class PromptGenerationAgent(dspy.Module):
    ...
    def forward(
        ...
        try:
            ...
            text_positions = self._organize_text_positions(
                caption, format_details.get("structure", {}).get("text_positions", [])
            )

            # Compile style guide
            style_guide = self._compile_style_guide(
                format_details.get("structure", {}), style_preferences if style_preferences else {}
            )
    ...
    def _apply_constraints(
        self, text: str, constraints: Dict[str, Any], text_positions: List[str]
    ) -> str:
        """
    ...
    def _apply_style_preferences(
        self, prompt: str, preferences: Dict[str, Any], base_style: str
    ) -> str:
        """
    ...
    def _compile_style_guide(
        self, format_structure: Dict[str, Any], style_preferences: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
