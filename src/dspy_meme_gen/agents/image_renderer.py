...
class ImageRenderingAgent(dspy.Module):
    ...
    def forward(
        ...
        try:
            ...
            if (
                caption
                and format_details
                and "text_overlay" in format_details.get("requirements", [])
            ):
    ...
    def _generate_stability_image(self, prompt: str) -> bytes:
        ...
        try:
            ...
            stability_api = client.StabilityInference(
                key=self.api_key, engine="stable-diffusion-v1-5"
            )
            response = stability_api.generate(prompt=prompt, width=512, height=512)
            return response.images[0].tobytes()
        except ImportError:
            raise RuntimeError(
                "stability_sdk not installed. Please install it to use Stability AI."
            )

    def _add_text_overlay(
        self, image_data: bytes, caption: str, format_details: Dict[str, Any]
    ) -> bytes:
    ...
    def add_text_overlay(self, image_data: bytes, caption: str, position: str = "bottom") -> bytes:
        """
    ...
    def upload_to_cloudinary(self, image_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            upload_result = cloudinary.uploader.upload(
                image_io, folder="meme_generator", resource_type="image"
            )
