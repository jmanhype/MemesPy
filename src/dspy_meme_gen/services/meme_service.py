...
class AsyncEventSourcedMemeService:
    ...
    async def get_memes(
        ...
        """
        logger.info(
            f"Getting memes: status={status}, topic={topic}, format={format}, limit={limit}, offset={offset}"
        )
    ...
    async def get_meme(self, meme_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if result:
                ...
                metadata = EventMetadata(aggregate_id=meme_uuid, aggregate_type="Meme")

                view_event = MemeViewed(meme_id=meme_uuid, view_source="api")
    ...
    async def create_meme(
        ...
        metadata = CommandMetadata(user_id=user_id, correlation_id=uuid4())

        command = GenerateMemeCommand(
            topic=topic, format=format_id, style=style, parameters=parameters or {}
        )
    ...
    async def score_meme(self, meme_id: str, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            command = ScoreMemeCommand(meme_id=meme_uuid)
            command.metadata = metadata
    ...
    async def delete_meme(
        self, meme_id: str, reason: str, user_id: Optional[str] = None, soft_delete: bool = True
    ) -> Dict[str, Any]:
        """
        Delete a meme.
        ...
        try:
            ...
            command = DeleteMemeCommand(
                meme_id=meme_uuid, deletion_reason=reason, soft_delete=soft_delete
            )
            command.metadata = metadata
    ...
    async def share_meme(
        self, meme_id: str, platform: str, method: str = "api", user_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            metadata = EventMetadata(aggregate_id=meme_uuid, aggregate_type="Meme", user_id=user_id)

            share_event = MemeShared(
                meme_id=meme_uuid, sharer_id=user_id, share_platform=platform, share_method=method
            )
