...
from opentelemetry.sdk.trace.sampling import TraceIdRatioBased, ParentBased, AlwaysOn, AlwaysOff
...
def init_telemetry(
    ...
    trace_provider = TracerProvider(
        resource=resource, sampler=ParentBased(root=TraceIdRatioBased(sampling_rate))
    )
    ...
    if otlp_endpoint:
        otlp_exporter = OTLPSpanExporter(
            endpoint=otlp_endpoint, insecure=settings.app_env != "production"
        )
    ...
    if otlp_endpoint:
        ...
        metric_readers.append(
            PeriodicExportingMetricReader(otlp_metric_exporter, export_interval_millis=10000)
        )

    # Add Prometheus reader
    prometheus_reader = PrometheusMetricReader(target_info=resource.attributes, registry=REGISTRY)
...
def trace_span(
    ...
    """
    tracer = get_tracer()
    with tracer.start_as_current_span(name, kind=kind, attributes=attributes, links=links) as span:
        try:
            yield span
    ...
    def __enter__(self):
        ...
        if self.trace_id and self.span_id:
            ...
            span_context = SpanContext(
                trace_id=int(self.trace_id, 16), span_id=int(self.span_id, 16), is_remote=True
            )
    ...
    def extract_from_headers(headers: Dict[str, str]) -> TracingContext:
        ...
        if span and span.get_span_context().is_valid:
            ...
            return TracingContext(
                trace_id=format(span_ctx.trace_id, "032x"), span_id=format(span_ctx.span_id, "016x")
            )
        return TracingContext()
...
class MetricsCollector:
    ...
    def __init__(self):
        ...
        self._request_duration = meter.create_histogram(
            MetricNames.REQUEST_DURATION, unit="ms", description="Request duration in milliseconds"
        )

        self._request_counter = meter.create_counter(
            MetricNames.REQUEST_COUNT, unit="requests", description="Total number of requests"
        )

        self._pipeline_duration = meter.create_histogram(
            MetricNames.PIPELINE_DURATION, unit="ms", description="Pipeline execution duration"
        )
        ...
        self._agent_duration = meter.create_histogram(
            MetricNames.AGENT_EXECUTION_DURATION, unit="ms", description="Agent execution duration"
        )

        self._cache_hits = meter.create_counter(
            MetricNames.CACHE_HIT_RATE, unit="hits", description="Cache hit count"
        )

        self._cache_misses = meter.create_counter(
            MetricNames.CACHE_MISS_COUNT, unit="misses", description="Cache miss count"
        )
    ...
    def record_request(self, duration_ms: float, method: str, endpoint: str, status_code: int):
    ...
    def record_pipeline_execution(self, duration_ms: float, stage: str, success: bool):
        """Record pipeline execution metrics."""
        attributes = {"stage": stage, "success": str(success)}
    ...
    def record_quality_score(self, score: float, format_type: str):
        ...
        attributes = {"format": format_type, "score_bucket": str(score_bucket)}
    ...
    def record_agent_execution(self, agent_name: str, duration_ms: float, success: bool):
        ...
        attributes = {"agent": agent_name, "success": str(success)}
