...
class ScoringAgent(dspy.Module):
    ...
    def forward(
        ...
        try:
            ...
            humor_result = self.humor_scorer(
                caption=caption,
    ...
    def _calculate_final_score(
        ...
        if appropriateness_check:
            appropriateness_factor = 1.0 if appropriateness_check.get("is_appropriate", True) else 0.5
    ...
    def _generate_improvement_suggestions(
        ...
        if scores.creativity_score < 0.7:
            suggestions.append(f"Boost creativity: {self._extract_suggestion(creativity_reasoning)}")
        if scores.shareability_score < 0.7:
            suggestions.append(f"Increase shareability: {self._extract_suggestion(shareability_reasoning)}")
