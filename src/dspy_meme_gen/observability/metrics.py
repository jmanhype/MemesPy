...
class ActorMetrics:
    ...
    def record_message_processed(
        self, message_type: str, duration_ms: float, success: bool = True, **attributes
    ):
...
class PipelineMetrics:
    ...
    def __init__(self):
        ...
        self._pipeline_duration = self.meter.create_histogram(
            name="pipeline.execution.duration", description="Pipeline execution duration", unit="ms"
        )
        ...
        self._stage_duration = self.meter.create_histogram(
            name="pipeline.stage.duration", description="Pipeline stage duration", unit="ms"
        )
    ...
    def start_pipeline(self, pipeline_id: str, pipeline_type: str, topic: str, format_type: str):
        ...
        with self._lock:
            ...
            attributes = {"pipeline_type": pipeline_type, "topic": topic, "format": format_type}
    ...
    def start_stage(self, pipeline_id: str, stage_name: str):
        ...
        with self._lock:
            if pipeline_id in self._active_pipelines:
                ...
                pipeline["stages"][stage_name] = {"start_time": time.time(), "completed": False}

    def complete_stage(self, pipeline_id: str, stage_name: str, success: bool, **stage_data):
        ...
        **pipeline_data,
        ...
        with self._lock:
            ...
            if quality_score is not None:
                quality_attributes = {"topic": pipeline["topic"], "format": pipeline["format"]}
    ...
    def get_pipeline_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            quality_scores = [
                p.get("quality_score") for p in self._completed_pipelines if p.get("quality_score")
            ]
            ...
            return {
                ...
                "total_refinements": sum(
                    p.get("refinement_count", 0) for p in self._completed_pipelines
                ),
            }
...
class SystemMetrics:
    ...
    def __init__(self):
        ...
        self._system_cpu = self.meter.create_histogram(
            name="system.cpu.usage", description="System CPU usage", unit="percent"
        )

        self._system_memory = self.meter.create_histogram(
            name="system.memory.usage", description="System memory usage", unit="MB"
        )

        self._system_disk = self.meter.create_histogram(
            name="system.disk.usage", description="System disk usage", unit="percent"
        )
        ...
        self._api_requests = self.meter.create_counter(
            name="system.api.requests.total", description="Total API requests", unit="requests"
        )

        self._api_duration = self.meter.create_histogram(
            name="system.api.request.duration", description="API request duration", unit="ms"
        )

        # Error metrics
        self._system_errors = self.meter.create_counter(
            name="system.errors.total", description="Total system errors", unit="errors"
        )
    ...
    def record_api_request(self, method: str, endpoint: str, status_code: int, duration_ms: float):
    ...
    def record_system_resources(self, cpu_percent: float, memory_mb: float, disk_percent: float):
    ...
    def record_error(self, error_type: str, component: str, severity: str = "error"):
        ...
        attributes = {"error_type": error_type, "component": component, "severity": severity}
...
class MetricsAggregator:
    ...
    def export_prometheus_metrics(self) -> str:
        ...
        for actor_name, actor_data in summary["actors"].items():
            ...
            lines.append(
                f'actor_messages_total{{actor="{actor_name}"}} {actor_data["total_messages_processed"]}'
            )
